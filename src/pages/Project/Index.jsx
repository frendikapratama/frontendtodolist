...
const IndexProject = () => {
  ...
  const [isUsersLoading, setIsUsersLoading] = useState(true); // Fix #7
  const [selectedProject, setSelectedProject] = useState(null);
  const [isFormOpen, setIsFormOpen] = useState(false);
  const [isFormDirty, setIsFormDirty] = useState(false); // Fix #3
  const [projectToDelete, setProjectToDelete] = useState(null);
  const [isFilterOpen, setIsFilterOpen] = useState(false);

  const [searchInput, setSearchInput] = useState(search);
  const [isSearchPending, setIsSearchPending] = useState(false); // Fix #1
  ...
  // Fix #4: tutup modal aktif dengan tombol Escape
  useEffect(() => {
  ...
  const totalProjects = summary.total ?? projectList.length; // Fix #2 (partial)
  ...
  // Fix #3: konfirmasi sebelum menutup jika form sedang dirty
  ...
  // Fix #6: status yang tidak dikenal ditampilkan apa adanya, bukan disamarkan jadi "Draft"
  ...
  // Helper untuk mendapatkan nama user/divisi terpilih
  // NOTE (#8): karena usersList dibatasi limit 100, PM di luar batch ini akan
  // tetap fallback ke "1 Terpilih". Perbaikan penuh butuh endpoint get-user-by-id.
    ...
    <div className="p-4 sm:p-6 max-w-7xl mx-auto space-y-6 bg-slate-950 min-h-screen text-slate-100 font-sans">
              ...
              <div className="space-y-1">
                ...
                <div className="relative">
                  <select
                    ...
                    className="w-full min-h-10 appearance-none bg-slate-950 text-slate-200 text-xs pl-3 pr-8 rounded-lg border border-white/10 focus:outline-none focus:border-blue-500 disabled:opacity-60 disabled:cursor-wait"
                  >
                    {/* Fix #7: state loading eksplisit, bukan dropdown kosong senyap */}
                    {isUsersLoading ? (
      ...
      <main className="bg-slate-900/60 border border-white/10 rounded-2xl overflow-hidden shadow-xl">
        {projectQuery.isLoading ? (
          ...
          // Fix #10: pesan error spesifik, bukan generik "cek koneksi" untuk semua kasus
          ...
          <>
            ...
            <div className="block md:hidden divide-y divide-white/5">
              {projectList.map((project) => {
                const clientParty = project.parties?.find(
                  (p) => p.role === "client",
                );
                const vendorParty = project.parties?.find(
                  (p) => p.role === "vendor",
                );
                ...
                return (
                  <div key={project._id} className="p-4 space-y-3">
                    ...
                    <div className="text-xs space-y-1.5 text-slate-300">
                      ...
                      {(clientParty || vendorParty) && (
                        <div className="flex items-center gap-2 text-slate-400">
                          <Building2 className="w-3.5 h-3.5 text-slate-500 shrink-0" />
                          <span className="truncate">
                            {clientParty?.party?.name || "-"} /{" "}
                            {vendorParty?.party?.name || "-"}
                          </span>
                        </div>
                      )}
                      ...
                      <div className="flex items-center gap-2 pt-0.5">
                        {project.sites?.length > 0 && (
                          <span className="inline-flex items-center gap-1 bg-white/5 px-2 py-0.5 rounded text-[10px] text-slate-400">
                            <MapPin className="w-3 h-3 text-slate-400" />
                            {project.sites.length} Site
                          </span>
                        )}
                        {project.divisionId?.length > 0 && (
                          <span className="inline-flex items-center gap-1 bg-white/5 px-2 py-0.5 rounded text-[10px] text-slate-400">
                            <Layers className="w-3 h-3 text-slate-400" />
                            {project.divisionId.length} Divisi
                          </span>
                        )}
                      </div>
            ...
            {/* TODO (#9): kolom belum bisa di-sort — butuh state sort + query param backend */}
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="py-4 px-4">
                          <div className="flex items-center gap-1.5">
                            {project.sites?.length > 0 && (
                              <span className="flex items-center gap-1 bg-white/5 px-2 py-0.5 rounded text-[11px] text-slate-400">
            ...
            {/* PAGINATION FOOTER */}
            {/* Fix #2 (partial): total sekarang akurat dari summary.total.
                Navigasi next/prev masih butuh page state dari useProject — lihat TODO. */}
            <div className="p-4 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-400">
              ...
              <div className="flex items-center gap-2">
                {/* TODO (#2): sambungkan ke page state nyata (mis. useProject harus expose
                    page, setPage, hasNextPage) supaya tombol ini benar-benar berfungsi */}
                <button
                  disabled
                  className="p-2 rounded-lg border border-white/10 bg-slate-900 text-slate-600 cursor-not-allowed"
                  title="Halaman Sebelumnya"
                >
                  <ChevronLeft className="w-4 h-4" />
                </button>
                <span className="text-slate-200 font-medium px-2">Hal 1</span>
                <button
                  disabled
                  className="p-2 rounded-lg border border-white/10 bg-slate-900 text-slate-600 cursor-not-allowed"
        ...
        >
          ...
          <div className="bg-slate-900 text-white rounded-2xl shadow-2xl border border-white/10 relative z-10 p-6 max-w-xl w-full max-h-[90vh] overflow-y-auto">
            ...
            <FormProject
              ...
              onDirtyChange={setIsFormDirty} // Fix #3: FormProject perlu memanggil ini saat field berubah
            />
          </div>
        </div>
      )}
