...
const SearchableFilter = ({
  ...
  <div className="space-y-1" ref={dropdownRef}>
    ...
    <div className="relative">
      ...
      {isOpen && (
        <div className="absolute left-0 right-0 top-full mt-1 bg-white border border-slate-200 rounded-xl shadow-2xl z-50 overflow-hidden animate-in fade-in slide-in-from-top-1 duration-150">
          <div className="p-2 border-b border-slate-200 bg-white/60">
            <div className="relative">
              ...
              items.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isSelected && (
                      <Check className="w-3.5 h-3.5 text-[#0E7490] shrink-0 ml-2" />
                    )}
                  </button>
                );
              })
      ...
      <div className="flex flex-col sm:flex-row gap-2.5 items-stretch sm:items-center justify-between">
        <div className="relative flex-1">
          ...
          {(isSearchPending || projectQuery.isFetching) &&
            !searchInput === false && (
              <Loader2 className="w-4 h-4 absolute right-9 top-1/2 -translate-y-1/2 text-[#0E7490] animate-spin" />
            )}
          {searchInput && (
            <button
              onClick={clearSearch}
              className="absolute right-3 top-1/2 -translate-y-1/2 p-1 text-slate-500 hover:text-slate-900"
        ...
        <div className="flex items-center gap-2">
          ...
          {hasActiveFilters && (
            <button
              ...
              className="min-h-11 px-3 py-2 rounded-xl border border-rose-100 bg-rose-50 text-rose-600 hover:bg-rose-100 text-xs sm:text-sm font-medium flex items-center gap-1.5 justify-center transition-all cursor-pointer"
