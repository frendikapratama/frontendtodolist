...
const ProjectParties = ({ parties }) => {
  ...
  return (
    <div className="flex items-start gap-2 text-slate-400 md:block md:space-y-1">
...
const ProjectRow = ({ project, onView, onEdit, onDelete }) => (
  <tr className="block md:table-row p-4 md:p-0 space-y-3 md:space-y-0 hover:bg-white/2 transition-colors group">
    <td className="block md:table-cell py-0 md:py-4 px-0 md:px-4 font-semibold text-white">
      <div className="flex items-start justify-between gap-2 md:block">
        <button
          type="button"
          onClick={() => onView(project)}
          className="text-left group-hover:text-blue-400 hover:underline transition-colors flex items-center gap-1.5 cursor-pointer min-w-0"
      ...
      {project.sites?.length ? (
        <div className="flex flex-wrap items-center gap-1 md:gap-1.5 md:max-w-[200px]">
          {project.sites.map((site, index) => (
            <span
              key={index}
              className="inline-flex items-center gap-1 bg-white/5 hover:bg-white/10 px-2 py-0.5 rounded text-[10px] md:text-[11px] text-slate-300 capitalize border border-white/5 font-medium transition-colors"
            >
    ...
    <td className="block md:table-cell py-0 md:py-4 px-0 md:px-4 whitespace-nowrap md:text-slate-400">
    ...
    <td className="block md:table-cell py-0 md:py-4 px-0 md:px-4 md:text-center pt-2 md:pt-4 border-t border-white/5 md:border-t-0">
      <div className="flex items-center justify-end md:justify-center gap-2 md:gap-1">
        ...
        <button
          ...
          className="min-h-11 md:min-h-0 px-3 md:px-0 py-1.5 md:p-2 rounded-lg bg-slate-800 md:bg-transparent text-slate-300 hover:text-white hover:bg-slate-700 md:hover:bg-amber-500/10 md:hover:text-amber-300 text-xs font-medium flex items-center gap-1 cursor-pointer transition-colors"
        >
        ...
        <button
          ...
          className="min-h-11 md:min-h-0 px-3 md:px-0 py-1.5 md:p-2 rounded-lg bg-rose-500/10 md:bg-transparent text-rose-400 hover:bg-rose-500/20 text-xs font-medium flex items-center gap-1 cursor-pointer transition-colors"
        >
...
const Pagination = ({
  ...
  <div className="p-4 border-t border-white/5 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-400">
    <div className="flex items-center gap-3">
      <span>
        Menampilkan{" "}
        <span className="text-white font-medium">{projectCount}</span> dari{" "}
        <span className="text-white font-medium">{totalProjects}</span> total
        proyek.
      </span>
    ...
    {query.isLoading ? (
      ...
      <>
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs text-slate-300 block md:table">
            <thead className="hidden md:table-header-group bg-white/5 text-slate-400 uppercase font-semibold text-[11px] tracking-wider border-b border-white/10">
            ...
            <tbody className="block md:table-row-group divide-y divide-white/5 md:divide-y">
