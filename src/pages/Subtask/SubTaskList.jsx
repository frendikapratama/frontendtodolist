...
const SubtaskList = ({
  ...
  const calculateAutoNote = (status, due_date, finish_date) => {
    if (status === "Done" && due_date && finish_date) {
      ...
      if (finishDate === dueDate) return "Completed - On Time";
  ...
  useEffect(() => {
    if (subtaskByTask.data) setLocalSubtasks(subtaskByTask.data);
  }, [subtaskByTask.data]);
  ...
  const handleAssignPic = useCallback(
    (subtaskId, email) => {
      ...
      assignPicMutation.mutate(
        ...
        { onSuccess: () => setPicPopup({ show: false, subtaskId: null }) },
      );
    },
    [assignPicMutation],
  );
  ...
  const handleReasonChange = useCallback((subtaskId, e) => {
  ...
  const handleDeletePIC = useCallback((subtaskId, userId) => {
    setConfirmDeletePIC({ show: true, subtaskId, userId });
  }, []);
  ...
  }, [confirmDeletePIC.subtaskId, confirmDeletePIC.userId, removePicMutation]);
      ...
      {displaySubtasks.map((s, index) => (
        ...
        >
            ...
            <div className="flex items-center gap-1 relative">
                  ...
                  {s.pic.slice(0, 3).map((picUser, idx) => {
                    ...
                    return (
                      <div key={idx} className="relative group hover:z-20 z-10 transition-all cursor-pointer">
                        ...
                        {/* Hanya authorized yang bisa hapus PIC */}
                        {isAuthorized && (
                          <button
              ...
              {picPopup.show && picPopup.subtaskId === s._id && (
                <PicPopup
                  ...
                  buttonRef={{ current: buttonRefs.current[`pic-${s._id}`] }}
                />
              )}
            </div>
          </div>
          ...
          <div className={`${columnWidths.action} px-6 py-3 flex items-center gap-2 justify-center shrink-0`}>
            ...
            {/* Hanya authorized yang bisa delete subtask */}
            {isAuthorized && showAddButton && (
