    ...
    return (
        <>
            ...
            <div className="space-y-6 animate-fadeIn">
                {/* Header */}
                <div>
                    <h3 className="text-xl font-bold text-gray-800 mb-2">{workspace.nama}</h3>
                ...
                <div className="grid grid-cols-2 gap-3">
                    {stats.map((stat, index) => (
                        <div
                            ...
                            style={{
                                animation: isAnimating ? `slideUp 0.5s ease-out ${index * 0.1}s both` : 'none'
                            }}
                        >
                            <p className="text-xs font-medium text-gray-600 mb-1">{stat.label}</p>
                            <p className={`text-2xl font-bold ${stat.color}`}>
                                {isAnimating ? <AnimatedNumber value={stat.value} duration={1000} /> : stat.value}
                            </p>
                        </div>
                    ))}
                </div>
